...
Bitmap.prototype.drawText = function(text, x, y, maxWidth, lineHeight, align) {
    ...
    //       So we use 'alphabetic' here.文字阴影
    if (text !== undefined) {
        var tx = x;
...
Graphics._makeErrorHtml = function(name, message) {
    return ('<font color="yellow" style="font-size: 31px;font-family: GameFont"><b>' +"进入武林时发生错误"+ '</b></font><br>' +
            '<font color="white" style="font-size: 14px;font-family: GameFont">' +"["+name+"]："+ message + "(截图报告小熊)"+'</font><br>');
};//报错反馈
...
Decrypter.readEncryptionkey = function(){
    if(sessionStorage.getItem("Fmii")==undefined){sessionStorage.setItem("Fmii","7a9168638d69d5de0f54581843a59af5")}
    this._encryptionKey = sessionStorage.getItem("Fmii").split(/(.{2})/).filter(Boolean);
};
